...
const MenusPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get(`${requestMethod}/restaurant/menu/find/menu/${data.store_id}/${params.category}`);
    ...
    <div className={`body-restaurant ${data.theme}`} >
      ...
      <div className='search-ctn'>
        <div className="search">
          <Link to={`/restaurant/${String(params.nameStore)}/homepage/${table_id}`}><IoChevronBackSharp size={30} color='black' style={{ marginLeft: '10px' }} /></Link>
